...
def Debe_Heredar_Filtros_De_Contexto(
    ...
    Categoria_Es_La_Misma = Categoria_Filtro == Filtros_Anteriores.get("category")
    
    if Genero_Filtro and not any([Categoria_Filtro and not Categoria_Es_La_Misma, Color_Filtro, Precio_Maximo_Filtro is not None, Talla_Filtro]) and len(Tokens_Del_Mensaje) <= 6:
        return True

    if Color_Filtro and not any([Categoria_Filtro and not Categoria_Es_La_Misma, Genero_Filtro, Precio_Maximo_Filtro is not None, Talla_Filtro]) and len(Tokens_Del_Mensaje) <= 6:
        return True

    if Talla_Filtro and not any([Categoria_Filtro and not Categoria_Es_La_Misma, Color_Filtro, Genero_Filtro, Precio_Maximo_Filtro is not None]) and len(Tokens_Del_Mensaje) <= 6:
        return True

    if Precio_Maximo_Filtro is not None and not any([Categoria_Filtro and not Categoria_Es_La_Misma, Color_Filtro, Genero_Filtro, Talla_Filtro]) and len(Tokens_Del_Mensaje) <= 6:
        return True
...
def Obtener_Respuesta_Principal(Id_De_Sesion, Mensaje_Usuario):
    ...
    if (Color_Filtro or Precio_Maximo_Filtro or Genero_Filtro or Categoria_Filtro or Talla_Filtro):
        if Etiqueta_Detectada in {"saludo", "fuera_de_dominio", None, "consulta_precio"}:
            Etiqueta_Detectada = "buscar_producto"
            
    # Si se detectó un precio máximo en un mensaje corto (ej: "y de 100 soles?"), forzar siempre búsqueda.
    if Precio_Maximo_Filtro and len(Mensaje_Normalizado.split()) <= 6:
        Etiqueta_Detectada = "buscar_producto"
    ...
    if Etiqueta_Detectada == "buscar_producto":
        ...
        if not Productos_Encontrados and not Palabras_Clave_Detectadas and not Categoria_Filtro and Color_Filtro:
            ...
            Respuesta_Final = "No encontré productos con esas características. He reiniciado los filtros para que veas otras opciones del catálogo:"
            Accion_De_Filtro = {
                "category": None,
                "color": None,
                "max_price": None,
        ...
        if Genero_Filtro:
            ...
            if Cantidad_Productos > 0:
                Texto_Natural = Obtener_Texto_Natural_De_Busqueda(Palabras_Clave_Detectadas)
                if Texto_Natural:
                    Texto_Cantidad = Texto_Natural
                    # Remover "en CALZADO" porque ya dice "zapatillas"
                    if Categoria_Filtro and f" en {Categoria_Filtro}" in Texto_De_Condicion:
                        Texto_De_Condicion = Texto_De_Condicion.replace(f" en {Categoria_Filtro}", "")
                else:
                ...
                Respuesta_Final = f"No encontré productos{Texto_De_Condicion}. Aquí tienes otras opciones del catálogo completo:"
                Accion_De_Filtro = {
                    "category": None,
                    "color": None,
                    "max_price": None,
    ...
    if not Respuesta_Final:
        if Categoria_Filtro or Color_Filtro or Genero_Filtro or Palabras_Clave_Detectadas or Talla_Filtro:
            ...
            if not Productos_Encontrados and not Palabras_Clave_Detectadas and not Categoria_Filtro and Color_Filtro:
                ...
                Respuesta_Final = "No encontré nada con esos términos. Te muestro opciones destacadas del catálogo general:"
                Accion_De_Filtro = {
                    "category": None,
                    "color": None,
                    "max_price": None,
