...
def Cargar_Lista_De_Productos_Desde_BD():
    try:
        productos_db = db.ejecutar_consulta("SELECT * FROM vista_productos_completa")
        if not productos_db:
            return []

        Lista_Normalizada = []
        for row in productos_db:
            colores_str = row.get("colores")
            if colores_str:
                colores_list = [c.strip() for c in str(colores_str).split(',') if c.strip()]
            else:
                colores_list = []
                
            color_princ = row.get("color_principal") or (colores_list[0] if colores_list else "")

            tallas_str = row.get("tallas")
            if tallas_str:
                tallas_list = [t.strip() for t in str(tallas_str).split(',') if t.strip()]
            else:
                tallas_list = []
            ...
            prod = {
                "id": row.get("id"),
                "name": row.get("nombre"),
                "price": float(row.get("precio")) if row.get("precio") is not None else 0.0,
                ...
                "image": row.get("imagen_url") or ""
            }
            
            Producto_Normalizado = Normalizar_Producto_Cargado(prod)
            if Producto_Normalizado is not None:
                Lista_Normalizada.append(Producto_Normalizado)
                
        return Lista_Normalizada
    except Exception as exc:
        print(f"[ERROR DB] No se pudo cargar catálogo desde MySQL: {exc}")
        return []
...
def Generar_Variantes_Lexicas_De_Termino(Termino):
    ...
    for variante_actual in list(Variantes):
        if variante_actual in Sinonimos:
            Variantes.update(Sinonimos[variante_actual])
